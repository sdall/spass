...
using Spass, Comonicon, JSON, GZip, CSV
...
This is a command line interface for the __Spass__ algorithm, which discovers a statistically significantly non-redundant set of patterns by leveraging a binomial redundancy test for a sequentially updated maximum entropy null-model.
...
_Example:_ `bin/spass.jl data.tsv labels.tsv --fdr --alpha=0.0001 > result.json`
...
- `--alpha`: Set the initial statistical significance level.
...
- `--max_factor_size`: Constraint the maximum number of patterns that each factor of the maximum entropy distribution can model. As inference complexity grows exponentially, the `max_factor_size` is bounded by `MAX_MAXENT_FACTOR_SIZE=12`.
- `--max_factor_width`: Constraint the maximum number of singletons that each factor can model.

# Flags

- `--fdr`: Adjust for the _False Discovery Rate_ instead of the Family-wise Error Rate.
- `--measure_time`: Ensure precise compile-time estimates during experiments.
"""
@main function spass(x, y                       = "";
                     alpha::Float64          = 0.05,
                     fdr::Bool               = false,
                     min_support::Int64      = 2,
    ...
    X, vocab = if any(e -> endswith(x, e), (".dat", ".dat.gz"))
        ...
        CSV.read(x, CSV.Tables.matrix; header=0, types=Bool), nothing
    end
    Y = !isnothing(y) && !isempty(y) && isfile(y) ? read_labels(y) : nothing

    adj = fdr ? Spass.FDR : Spass.FWER

    if measure_time && Threads.nthreads() > 1
        Spass.spass(adj, X, Y; alpha=alpha, min_support=min_support, max_seconds=oftype(max_seconds, 0),
                    max_discoveries=max_discoveries, max_factor_size=max_factor_size, max_factor_width=max_factor_width)
    end

    t = @timedv p = Spass.spass(adj, X, Y; alpha=alpha, min_support=min_support, max_seconds=max_seconds,
                                max_discoveries=max_discoveries, max_factor_size=max_factor_size, max_factor_width=max_factor_width)

    out = isnothing(vocab) ? e -> Set(e) : e -> Set(vocab[i] for i in e)
    S = p isa Vector ? [out.(Spass.patterns(q)) for q in p] : out.(Spass.patterns(p))
